...
export const Checkout: React.FC<CheckoutProps> = ({ items, onComplete, onCancel }) => {
  ...
  // Abandoned-checkout lead tracking: as soon as the customer submits their
  // details, we save a lightweight "abandoned" record per item so the admin
  // panel can see (and follow up with) customers who never finish paying.
  // These same records get upgraded in place (never duplicated) as the
  // customer progresses further through checkout.
  const [leadRecords, setLeadRecords] = useState<{ id: string; firebaseKey?: string }[]>([]);
  ...
  // Payment Links (admin-configured, one per exact payable amount).
  ...
  const createTransactions = (status: 'success' | 'failed' | 'pending', txnId: string): Transaction[] => {
    return items.map((item, idx) => ({
      id: leadRecords[idx]?.id || `ORD-${Math.floor(100000 + Math.random() * 900000)}`,
      firebaseKey: leadRecords[idx]?.firebaseKey, // reuse the lead record so it's updated, not duplicated
      transactionId: txnId, 
      courseId: item.id,
  ...
  // Fires the moment the customer submits their name/email/phone and moves
  // to the payment step - captured as an "abandoned" lead immediately, so
  // even if they close the tab here, admin still has their contact info.
  const handleDetailsSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setStep('payment');
    const leads: Transaction[] = items.map(item => ({
      ...
      customerPhone: formData.phone,
      status: 'abandoned',
      checkoutStage: 'DETAILS_SUBMITTED',
    }));
    Promise.all(leads.map(l => saveTransactionToDb(l))).then(keys => {
      setLeadRecords(leads.map((l, i) => ({ id: l.id, firebaseKey: keys[i] || undefined })));
    });
  };
  ...
  // Customer confirms they have completed payment via the payment link.
  // This does NOT verify the payment automatically - it only creates a
  // "pending" order which still requires manual/admin verification,
  // exactly like the previous manual UPI flow.
  const handlePaymentDoneClick = (e: React.FormEvent) => {
    e.preventDefault();
    setStep('verification');
    setTimeout(() => {
       const pendingTxns = createTransactions('pending', 'PAYMENT-LINK-VERIFY');
       onComplete(pendingTxns); 
    }, 1500);
  };
                ...
                <div className="md:col-span-4 bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden h-fit">
                   ...
                   <div className="divide-y divide-gray-100">
                      ...
                      >
                         ...
                         <div>
                            <p className="font-bold text-gray-800 text-sm">UPI / Pay Now</p>
                         ...
                         {selectedMethod === 'upi' && (
                            <div className="max-w-md mx-auto text-center">
                               <div className="bg-indigo-50 border border-indigo-100 rounded-xl p-6 mb-6">
                                  <Smartphone size={36} className="text-indigo-600 mx-auto mb-3" />
                                  <p className="text-sm text-gray-600 mb-1">
                                     Click below to pay securely using UPI, Cards, Netbanking & Wallets.
                                  </p>
                                  <p className="text-2xl font-black text-gray-900 mt-2">{formatCurrency(finalTotal)}</p>
                               </div>
                               ...
                               {matchedPaymentLink ? (
                                 ...
                                 <p className="text-[11px] text-red-500 mt-3 flex items-center justify-center gap-1">
                                    <AlertCircle size={12}/> Online payment isn't set up for this amount yet. Please contact support before paying.
                                 </p>
                               )}
