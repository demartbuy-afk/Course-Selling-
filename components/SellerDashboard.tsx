...
export const SellerDashboard: React.FC<SellerDashboardProps> = ({ courses, transactions, onAddCourse, onEditCourse, onDeleteCourse, onLogout, onShareCourse, onPreviewWebsite }) => {
  ...
  // Payment Links State (one payment link per exact payable amount)
  ...
  // --- Payment Links (amount -> URL) ---
  const addPaymentLinkRow = () => {
    setPaymentLinks(prev => [...prev, { id: `pl_${Date.now()}`, amount: 0, url: '', label: '' }]);
  };

  const updatePaymentLinkRow = (id: string, field: keyof PaymentLink, value: string) => {
    setPaymentLinks(prev => prev.map(link => {
      if (link.id !== id) return link;
      if (field === 'amount') return { ...link, amount: Number(value) || 0 };
  ...
  const handleSavePaymentLinks = async () => {
    const invalid = paymentLinks.some(l => !l.url.trim() || l.amount <= 0);
    if (invalid) {
      alert("Every payment link needs a valid amount (greater than 0) and a URL.");
      return;
    }
           ...
           {activeTab === 'settings' && (
              <div className="p-8 max-w-2xl">
                       ...
                       <div key={link.id} className="flex flex-col sm:flex-row gap-2 items-start sm:items-center bg-gray-50 border border-gray-200 rounded-lg p-3">
                          <div className="w-full sm:w-32">
                             ...
                             <input
                               ...
                               placeholder="499"
                               value={link.amount || ''}
                               onChange={(e) => updatePaymentLinkRow(link.id, 'amount', e.target.value)}
                             />
                          </div>
                          <div className="flex-1 w-full">
                             <label className="block text-xs font-bold text-gray-500 mb-1">Payment Link URL</label>
                             <input
                               type="text"
                 ...
                 <div className="flex flex-wrap items-center gap-3 mb-10">
                    <Button onClick={addPaymentLinkRow} className="bg-white border border-indigo-200 text-indigo-600 hover:bg-indigo-50 flex items-center gap-1">
                       <PlusCircle size={16}/> Add Payment Link
                    </Button>
                    <Button onClick={handleSavePaymentLinks} disabled={isSavingPaymentLinks}>
                       {isSavingPaymentLinks ? 'Saving...' : 'Save Payment Links'}
                    </Button>
                 </div>
