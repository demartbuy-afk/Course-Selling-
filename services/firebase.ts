...
// 4b. Update specific fields on an existing transaction record (used to
// upgrade an "abandoned" checkout lead as the customer progresses further,
// instead of creating a duplicate record).
export const updateTransactionFields = async (firebaseKey: string, fields: Partial<Transaction>) => {
  try {
    const sanitizedFields = JSON.parse(JSON.stringify(fields));
    await update(ref(db, `transactions/${firebaseKey}`), sanitizedFields);
    return true;
  } catch (error) {
...
// 7b. Payment Links (amount -> payment link URL mapping)
// Lets the admin add a dedicated payment link for every price point
// (full price, and every coupon-discounted price) so Checkout can send the
// customer to the link that matches the exact amount they owe.
export const fetchPaymentLinks = async (): Promise<PaymentLink[]> => {
  try {
    const snapshot = await get(child(ref(db), `paymentLinks`));
    if (snapshot.exists()) {
